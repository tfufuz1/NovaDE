...
pub struct FrameRenderer {
    ...
    texture: Option<Texture>, // User-loaded texture
    default_sampler: vk::Sampler, // Default sampler for compute output or when no texture is loaded
...
impl FrameRenderer {
    ...
    pub fn new(
        ...
        info!("Creating FrameRenderer with Default Sampler, Pipeline Cache, Compute Pass, and PushConstants support...");
        ...
        info!("PipelineCache created: {:?}", pipeline_cache);
        ...
        let sampler_create_info = vk::SamplerCreateInfo::builder()
            ...
            .mipmap_mode(vk::SamplerMipmapMode::LINEAR) // Relevant if sampling from mipmapped images
            .min_lod(0.0).max_lod(1.0) // Max LOD 1.0 for non-mipmapped or single-level compute output
            .anisotropy_enable(false) // Basic sampler
            .border_color(vk::BorderColor::FLOAT_OPAQUE_BLACK)
            .unnormalized_coordinates(false);
        let default_sampler = unsafe { logical_device_raw.create_sampler(&sampler_create_info, None) }?;
        info!("Default Sampler created: {:?}", default_sampler);
        ...
        for _ in 0..MAX_FRAMES_IN_FLIGHT {
            let (img, alloc, view) = texture::Texture::new_storage_image(
                &logical_device_raw, &allocator, surface_swapchain.extent().width, // Pass device_raw
        ...
        for _ in 0..MAX_FRAMES_IN_FLIGHT {
            let (buf, alloc, info) = allocator.create_buffer(
                &vk::BufferCreateInfo::builder().size(ubo_size).usage(vk::BufferUsageFlags::UNIFORM_BUFFER),
                &vk_mem::AllocationCreateInfo { usage: vk_mem::MemoryUsage::CpuToGpu,
                    flags: vk_mem::AllocationCreateFlags::MAPPED | vk_mem::AllocationCreateFlags::HOST_ACCESS_SEQUENTIAL_WRITE, ..Default::default() },
            )?;
            uniform_buffers.push(buf); uniform_buffer_allocations.push(alloc); uniform_buffer_mapped_pointers.push(info.get_mapped_data_mut());
        }
        ...
        for i in 0..MAX_FRAMES_IN_FLIGHT {
            let ubo_info = vk::DescriptorBufferInfo::builder().buffer(uniform_buffers[i]).offset(0).range(ubo_size);
            let compute_out_sampler_info = vk::DescriptorImageInfo::builder()
                .image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL)
                .image_view(compute_output_image_views[i])
                .sampler(default_sampler); // Use the default sampler here
            let writes = [
                vk::WriteDescriptorSet::builder().dst_set(graphics_descriptor_sets[i]).dst_binding(0)
                    .descriptor_type(vk::DescriptorType::UNIFORM_BUFFER).buffer_info(&[ubo_info.build()]).build(),
        ...
        info!("FrameRenderer initialization complete.");
        ...
        Ok(Self {
            ...
            uniform_buffers, uniform_buffer_allocations, uniform_buffer_mapped_pointers,
    ...
    pub fn update_uniform_buffer(&mut self, frame_index: usize, data: UniformBufferObject) -> Result<()> {
        if frame_index >= MAX_FRAMES_IN_FLIGHT { 
            let msg = format!("Invalid frame_index {} for UBO update (MAX_FRAMES_IN_FLIGHT is {}).", frame_index, MAX_FRAMES_IN_FLIGHT);
            error!("{}", msg); return Err(VulkanError::InitializationError(msg)); 
        }
        let ptr = self.uniform_buffer_mapped_pointers[frame_index];
        if ptr.is_null() { 
            let msg = format!("UBO for frame_index {} is not mapped or pointer is null.", frame_index);
            error!("{}", msg); return Err(VulkanError::InitializationError(msg)); 
        }
        unsafe { std::ptr::copy_nonoverlapping(&data, ptr as *mut UniformBufferObject, 1); }
        Ok(())
    }

    pub fn load_texture(
        &mut self, image_path: &str, physical_device_info: &PhysicalDeviceInfo,
        transfer_queue: vk::Queue, vulkan_instance: &VulkanInstance, // Changed graphics_queue to transfer_queue for clarity
    ) -> Result<()> {
        ...
        let new_texture_obj = texture::Texture::new_from_file(
            &self.logical_device_raw, // Pass ash::Device directly
            physical_device_info, &self.allocator, self.command_pool,
            transfer_queue, image_path, vulkan_instance.raw(),
        )?;
        info!("New texture {} loaded. Updating compute descriptor sets (binding 0).", image_path);
        ...
        for i in 0..MAX_FRAMES_IN_FLIGHT {
            let input_image_info = vk::DescriptorImageInfo::builder()
                ...
                .image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL);
            let input_write = vk::WriteDescriptorSet::builder()
                .dst_set(self.compute_descriptor_sets[i]).dst_binding(0)
                .descriptor_type(vk::DescriptorType::COMBINED_IMAGE_SAMPLER)
                .image_info(&[input_image_info.build()]).build();
            unsafe { self.logical_device_raw.update_descriptor_sets(&[input_write], &[]); }
        }
        debug!("All compute descriptor sets (binding 0) updated for new input texture.");
        // Also update graphics descriptor sets if they were using a null sampler before
        for i in 0..MAX_FRAMES_IN_FLIGHT {
             let ubo_info = vk::DescriptorBufferInfo::builder().buffer(self.uniform_buffers[i]).offset(0).range(std::mem::size_of::<UniformBufferObject>() as vk::DeviceSize);
             let compute_out_sampler_info = vk::DescriptorImageInfo::builder()
                .image_layout(vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL)
                .image_view(self.compute_output_image_views[i])
                .sampler(new_texture_obj.sampler); // Now use the loaded texture's sampler
            let writes = [
                vk::WriteDescriptorSet::builder().dst_set(self.graphics_descriptor_sets[i]).dst_binding(0)
                    .descriptor_type(vk::DescriptorType::UNIFORM_BUFFER).buffer_info(&[ubo_info.build()]).build(),
                vk::WriteDescriptorSet::builder().dst_set(self.graphics_descriptor_sets[i]).dst_binding(1)
                    .descriptor_type(vk::DescriptorType::COMBINED_IMAGE_SAMPLER).image_info(&[compute_out_sampler_info.build()]).build(),
            ];
            unsafe { self.logical_device_raw.update_descriptor_sets(&writes, &[]); }
        }
        debug!("Graphics descriptor sets (binding 1) updated to use new texture's sampler for compute output.");
    ...
    fn record_graphics_pass(
        &self, command_buffer: vk::CommandBuffer, image_index: u32, current_frame_idx_for_descriptors: usize,
    ) -> Result<()> {
        // ... (rest of the method remains the same, already documented)
        let clear_values = [
        ...
        unsafe {
            ...
            self.logical_device_raw.cmd_bind_descriptor_sets(
                ...
                0, &[self.graphics_descriptor_sets[current_frame_idx_for_descriptors]], &[],
            );

            let push_constant_data = GraphicsPushConstants {
                tint_color: [ (self.current_frame_index as f32 * 0.1).sin() * 0.5 + 0.5, 0.7, (self.current_frame_index as f32 * 0.05).cos() * 0.5 + 0.5, 1.0 ],
                scale: 0.5 + ((self.current_frame_index as f32 * 0.02).sin() * 0.25),
            };
    ...
    pub fn draw_frame(
        ...
        // ... (rest of the method remains the same, already documented and uses Result)
        let ubo_data = UniformBufferObject { color_multiplier: [1.0, 1.0, 1.0, 1.0] };
            ...
            Err(vk_err) => { 
                ...
                match vulkan_err {
                    VulkanError::SwapchainOutOfDate | VulkanError::SurfaceLost => {
                        warn!("Swapchain is out of date or surface lost during acquire. Triggering recreation. Error: {:?}", vk_err);
        ...
        if self.texture.is_some() {
            ...
            let input_texture_barrier = vk::ImageMemoryBarrier::builder()
                ...
                .image(texture_ref.image).subresource_range(vk::ImageSubresourceRange {
                    aspect_mask: vk::ImageAspectFlags::COLOR, base_mip_level: 0, level_count: texture_ref.mip_levels,
                    base_array_layer: 0, layer_count: 1,
                }).src_queue_family_index(vk::QUEUE_FAMILY_IGNORED).dst_queue_family_index(vk::QUEUE_FAMILY_IGNORED);
            unsafe { self.logical_device_raw.cmd_pipeline_barrier(current_command_buffer,
                vk::PipelineStageFlags::FRAGMENT_SHADER, vk::PipelineStageFlags::COMPUTE_SHADER,
                vk::DependencyFlags::empty(), &[], &[], &[input_texture_barrier.build()])};
            ...
            let out_to_general_barrier = vk::ImageMemoryBarrier::builder()
                ...
                .image(compute_output_image).subresource_range(vk::ImageSubresourceRange {
            ...
            unsafe { self.logical_device_raw.cmd_pipeline_barrier(current_command_buffer,
                ...
                vk::DependencyFlags::empty(), &[], &[], &[out_to_general_barrier.build()])};

            unsafe {
                self.logical_device_raw.cmd_bind_pipeline(current_command_buffer, vk::PipelineBindPoint::COMPUTE, self.compute_pipeline);
                self.logical_device_raw.cmd_bind_descriptor_sets(current_command_buffer, vk::PipelineBindPoint::COMPUTE,
                    self.compute_pipeline_layout.raw, 0, &[self.compute_descriptor_sets[self.current_frame_index]], &[]);
            ...
            unsafe { self.logical_device_raw.cmd_pipeline_barrier(current_command_buffer,
                ...
                vk::DependencyFlags::empty(), &[], &[], &[out_to_shader_read_barrier.build()])};
        } else { 
            let compute_output_image = self.compute_output_images[self.current_frame_index];
            let out_to_shader_read_barrier = vk::ImageMemoryBarrier::builder()
        ...
        self.record_graphics_pass(current_command_buffer, image_index, self.current_frame_index)?;
    ...
    pub fn recreate_swapchain(
        ...
        for _ in 0..MAX_FRAMES_IN_FLIGHT {
            let (img, alloc, view) = texture::Texture::new_storage_image(&self.logical_device_raw, &self.allocator, // Pass device_raw
        ...
        let ubo_size = std::mem::size_of::<UniformBufferObject>() as vk::DeviceSize;
        // let default_sampler = self.texture.as_ref().map_or(vk::Sampler::null(), |t| t.sampler);
        // Use self.default_sampler instead of possibly null sampler from self.texture
        ...
        for i in 0..MAX_FRAMES_IN_FLIGHT {
            ...
            let ubo_info = vk::DescriptorBufferInfo::builder().buffer(self.uniform_buffers[i]).offset(0).range(ubo_size);
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            ...
            if self.default_sampler != vk::Sampler::null() {
                self.logical_device_raw.destroy_sampler(self.default_sampler, None);
            }
