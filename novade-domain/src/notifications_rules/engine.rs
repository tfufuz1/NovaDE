...
use crate::user_centric_services::notifications_core::types::{Notification, NotificationAction as CoreNotificationAction, NotificationUrgency};
...
mod tests {
    use super::*;
    use crate::notifications_rules::persistence_iface::MockNotificationRulesProvider;
    use crate::global_settings::{MockGlobalSettingsService, SettingPathParseError};
    use serde_json::json;
    use crate::user_centric_services::notifications_core::types::NotificationUrgency;
    ...
    impl GlobalSettingsService for TestMockGlobalSettingsService {
        ...
        fn get_setting(&self, path: &SettingPath) -> Result<serde_json::Value, GlobalSettingsError> {
            self.settings.blocking_read().get(&path.to_string()).cloned().ok_or_else(|| GlobalSettingsError::PathNotFound { path: path.clone() })
        }
    ...
    async fn test_process_notification_setting_is_true_condition() {
        ...
        let setting_path_str = "dnd.is_enabled";
        settings_service.set_json_setting(setting_path_str, json!(true)).await;
        let rule_id = Uuid::new_v4();
        let rules = vec![NotificationRule {id: rule_id, name: "SettingTrueTest".into(), is_enabled: true, condition: RuleCondition::SettingIsTrue(SettingPath::from_str_unsafe_for_testing(setting_path_str)), actions: vec![RuleAction::SuppressNotification], ..Default::default()}];
        mock_rules_provider.expect_load_rules().times(1).returning(move || Ok(rules.clone()));
